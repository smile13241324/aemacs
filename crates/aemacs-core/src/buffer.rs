...
impl Buffer {
    ...
    #[must_use]
    pub fn new() -> Self {
        Self { content: Rope::new(), path: None, dirty: false }
    }
    ...
    pub fn from_file(path: PathBuf) -> Result<Self> {
        ...
        Ok(Self { content, path: Some(path), dirty: false })
    }
    ...
    pub fn save(&mut self) -> Result<()> {
        let path = self.path.as_ref().context("Cannot save a buffer with no path (Scratchpad)")?;
